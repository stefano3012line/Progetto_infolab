...
void reader(std::vector<variable> var, std::unordered_map<std::string,int> variable_position);
...
std::vector<std::vector<int>> line_to_parents_id( std::vector<std::string>& node_parents, std::unordered_map<std::string,int>& variable_position, std::vector<variable>& var);//converte le linee della cpt di un nodo negli id dei suoi parenti

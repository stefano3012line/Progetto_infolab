...
std::vector<variable> read_var(std::string filename){//caricare un vettore di variabili a partire dal bif
    ...
    int n_values;
    int i=0,parents_number=0,line_number=1;
    ...
    ///////BLOCCO PARENTS 
        if (line.find("probability(") != std::string::npos) {//ottiene i parenti di una certa variabile 
            ...
            if (parents_number == 0 ){
                ...
                cpt_vector.clear();// potrei ridefinire per fare più veloce
                
            }
            else{
                    for (size_t i = 0; i < parents_number; i++)
                    {
                        line_number*=var[variable_position[var[variable_position[var_name]].parents[i]]].values.size();
                    }
                for (size_t k = 0; k < line_number; k++){
                    std::getline(file, line);
                    line.erase(0,line.find(")")+1);
                    while (line.find(',') != std::string::npos)
                    {
